...
- **Granular Roles & Permissions Management System** (October 2025): Complete engine-level RBAC with 72 fine-grained permissions across 18 resource sections (All Users, All Orgs, All Entity, All DataSets, All Modules, All Apps, All Hubs, All Media, All Themes, All Integrations, Pricing Plans, Roles & Permissions, Servers & Backups, Help Centre, Engine CMS, System & Security, Analytics, Global Settings). Features table-based permission matrix UI with CRUD checkboxes (View, Create, Edit, Delete), idempotent seeding service, 3 default roles (Super Admin, Admin, Viewer), and protected API routes with `requirePermission` middleware. Permissions identified by resource-action pairs (e.g., 'roles-permissions:view').
